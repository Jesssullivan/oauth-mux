...
test "neutralize is fail-closed when a token marker rides in the scope passthrough" {
    const a = testing.allocator;
    const pad = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
    // valid token prefixes (the prefix gate passes), but a marker is smuggled into
    // the echoed `scope` field — the final-line marker check must drop it.
    const body = try std.fmt.allocPrint(
        a,
        "{{\"access_token\":\"{s}{s}\",\"refresh_token\":\"{s}{s}\",\"expires_in\":1," ++
            "\"scope\":\"user:inference sk-ant-ort01-LEAK\",\"token_type\":\"Bearer\"}}",
        .{ cas.access_token_prefix, pad, cas.refresh_token_prefix, pad },
    );
    defer a.free(body);
    try testing.expectError(error.MalformedCassette, cas.neutralizeSuccessBody(a, .refresh, body));
}
...
test "Replayer routing is anchored: a grant_type substring in a param value can't misroute" {
    ...
    // An unanchored substring matcher would route this to exchange (-> HttpFailed,
    // no exchange entry); the anchored matcher correctly routes to refresh.
    const body = "grant_type=refresh_token&redirect_uri=http://x?grant_type=authorization_code";
    const out = try cas.Replayer.post(a, &rp, cas.token_endpoint, body, cas.form_content_type);
    defer a.free(out);
    try testing.expect(std.mem.indexOf(u8, out, cas.refresh_token_prefix) != null);
    try testing.expect(rp.last_miss == null);
}

test "Cassette.parse: empty/whitespace -> 0 entries; garbage line -> MalformedCassette (no panic)" {
    const a = testing.allocator;
    var empty = try cas.Cassette.parse(a, "\n   \n");
    defer empty.deinit();
    try testing.expectEqual(@as(usize, 0), empty.entries.len);
    try testing.expectError(error.MalformedCassette, cas.Cassette.parse(a, "{ truncated"));
}
