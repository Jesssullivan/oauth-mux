...
fn renderAccountHealthHtml(allocator: std.mem.Allocator, accounts: []const AccountHealth) std.mem.Allocator.Error![]const u8 {
    ...
    for (accounts, 0..) |acc, idx| {
        const w = buf.writer();
        // Every operator/account-derived field is HTML-escaped (no raw {s}).
        try w.writeAll("      <div class=\"account-card\">\n        <div class=\"account-header\">\n          <div>\n            <div class=\"account-title\">");
        try writeHtmlEscaped(w, acc.provider);
        try w.writeAll("</div>\n            <div class=\"account-subtitle\">");
        try writeHtmlEscaped(w, if (acc.email_hint_masked) |hint| hint else "(email unavailable)");
        try w.writeAll("</div>\n          </div>\n          <span class=\"route-state ");
        try writeHtmlEscaped(w, acc.route_state);
        try w.writeAll("\">");
        try writeHtmlEscaped(w, acc.route_state);
        try w.writeAll("</span>\n        </div>\n");
        ...
        if (acc.reauth_in_progress) {
            ...
            // The provider is carried in an HTML-escaped data- attribute and read
            // via dataset in JS — NOT string-concatenated into an onclick handler
            // (which HTML-escaping alone cannot make safe in a JS-string context).
            const w2 = buf.writer();
            try w2.writeAll("        <div class=\"account-actions\">\n          <button class=\"btn-primary\" data-provider=\"");
            try writeHtmlEscaped(w2, acc.provider);
            try w2.print("\" data-selector=\"{d}\" onclick=\"startReauth(this)\">Re-login</button>\n          <span class=\"reauth-hint\">Confirm identity <strong>", .{idx});
            try writeHtmlEscaped(w2, acc.provider);
            try w2.writeAll(":");
            try writeHtmlEscaped(w2, if (acc.email_hint_masked) |hint| hint else "(email unavailable)");
            try w2.writeAll("</strong> and use a PRIVATE / INCOGNITO window.</span>\n        </div>\n");
        }
    ...
    try buf.appendSlice(
        ...
        \\    async function startReauth(btn) {
        \\      const provider = btn.dataset.provider;
        \\      const selector = btn.dataset.selector;
        \\      try {
        \\        const response = await fetch(`/reauth/${encodeURIComponent(provider)}/${encodeURIComponent(selector)}`, { method: 'POST' });
        ...
        \\        try {
        \\          const response = await fetch(`/reauth/${encodeURIComponent(provider)}/${encodeURIComponent(selector)}`);
        \\          const data = await response.json();
...
/// CSRF gate for state-changing requests. The Host check alone does NOT stop CSRF:
/// a page the operator visits can `fetch('http://127.0.0.1:PORT/reauth/...', {method:'POST'})`
/// and the browser sends `Host: 127.0.0.1` (loopback → host check passes). So a
/// mutating route additionally requires the `Origin` to be absent (a non-browser
/// client such as curl/tooling — not a CSRF vector) OR an `http://` loopback
/// origin (a genuine same-origin request from the console). A cross-origin browser
/// POST (`Origin: http://evil.com`, or any `https://`/non-loopback) is refused.
fn isLoopbackOrigin(origin_header: []const u8) bool {
    if (origin_header.len == 0) return true; // non-browser client: not a CSRF vector
    const scheme = "http://";
    if (!std.mem.startsWith(u8, origin_header, scheme)) return false; // https/other → refuse
    return isLoopbackHost(origin_header[scheme.len..]);
}

/// Write `s` to `w` with HTML metacharacters escaped (`& < > " '`). Used on EVERY
/// operator/account-derived field that lands in the rendered HTML, so a poisoned
/// provider key or a markup-bearing email hint cannot inject script into the
/// console origin. Values placed into JS contexts use `data-` attributes (read via
/// `dataset`), not string concatenation, so this HTML-escaping is sufficient.
...
fn resolveReauthTarget(
    ...
    // Index-only: a non-numeric / out-of-range selector is rejected rather than
    // passed through verbatim, so the POST can only ever target an account that
    // is actually in the bounds-checked seam list (no attacker-chosen literal
    // account string reaches enqueueReauth).
    if (!target.selector_is_index) return null;
...
pub fn routeRequest(
    ...
    if (std.mem.startsWith(u8, path, "/reauth/")) {
        if (method == .POST) {
            // CSRF: the POST mutates state, so it must originate same-origin
            // (loopback) or from a non-browser client. A cross-origin browser
            // POST is refused even though the Host header is loopback.
            if (!isLoopbackOrigin(origin_header)) {
                const body = try allocator.dupe(u8, "{\"error\":\"cross_origin_refused\"}");
                return Response{ .status = 403, .content_type = "application/json", .body = body, .allocator = allocator };
            }
            return try handlePostReauth(allocator, path, seams);
        }
...
pub fn parseRequest(raw: []const u8) ?ParsedRequest {
    ...
    while (lines.next()) |line| {
        ...
        if (std.ascii.eqlIgnoreCase(name, "host")) {
            ...
            origin = std.mem.trim(u8, line[colon + 1 ..], " \t");
        }
    }

    return .{ .method = method, .target = target, .host = host, .origin = origin };
}
...
pub fn serve(allocator: std.mem.Allocator, port: u16, seams: Seams) !void {
    ...
    while (true) {
        ...
        const resp = routeRequest(allocator, parsed.method, parsed.target, parsed.host, parsed.origin, seams) catch continue;
        defer resp.deinit();
