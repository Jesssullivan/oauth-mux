...
//! NOTE: the repo's declarative `claude_def` (src/provider_schema.zig:683) +
//! `oauth.zig:115` currently carry the WRONG token endpoint (`claude.ai/…`);
//! fixing those is coordinated work (shared files), not part of this module.
//!
...
pub fn parseTokenEndpointResponse(
    ...
    return .{
        ...
        .expires_at_ms = now_ms + w.expires_in * std.time.ms_per_s,
        .scopes = scopes,
...
/// Serialize tokens to the Claude credential file shape. Token/scope/subscription
/// values are constrained OAuth charsets (`[A-Za-z0-9._:-]`) with no JSON
/// metacharacters, so they are emitted without escaping; the adapter validates
/// the token prefixes before writing.
pub fn buildCredentialJson(allocator: std.mem.Allocator, t: ClaudeTokens) ![]u8 {
    var scopes_arr = std.ArrayList(u8).init(allocator);
    defer scopes_arr.deinit();
    try scopes_arr.append('[');
    if (t.scopes) |sc| {
        var it = std.mem.splitScalar(u8, sc, ' ');
        var first = true;
        while (it.next()) |s| {
            if (s.len == 0) continue;
            if (!first) try scopes_arr.append(',');
            first = false;
            try scopes_arr.append('"');
            try scopes_arr.appendSlice(s);
            try scopes_arr.append('"');
        }
    }
    try scopes_arr.append(']');

    const sub = t.subscription_type orelse "";
    return std.fmt.allocPrint(
        allocator,
        "{{\"claudeAiOauth\":{{\"accessToken\":\"{s}\",\"refreshToken\":\"{s}\"," ++
            "\"expiresAt\":{d},\"scopes\":{s},\"subscriptionType\":\"{s}\"}}}}",
        .{ t.access_token, t.refresh_token, t.expires_at_ms, scopes_arr.items, sub },
    );
}
