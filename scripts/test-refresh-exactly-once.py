...
fresh token without spending a refresh token.
...
class TokenEndpoint(http.server.BaseHTTPRequestHandler):
    ...
    valid_rt = "RT-0"
    rotated_rt = "RT-1"
    rotated_at: str = ""  # rotated access token, filled in main()
...
def racer(bin_path, env, results, idx):
...
def main():
    ...
    threads = [
        threading.Thread(target=racer, args=(args.bin, env, results, i))
