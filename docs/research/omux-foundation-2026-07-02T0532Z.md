...
| D6 | "Remove `oauth-mux/WORKSPACE.bazel`" (a file) | True, and **worse**: `.bazelrc` carries `common --noenable_bzlmod`, i.e. the bounded TIN-2105 Bazel surface runs in *legacy WORKSPACE mode* despite `MODULE.bazel`. Bzlmod-only means deleting the file **and** the flag. (resolved by #426: WORKSPACE.bazel deleted, .bazelrc bzlmod-only) | Handled on the `jess/tin-2105-*` branch. |
...
- **Bazel posture**: `MODULE.bazel` (module `oauth_mux` 0.1.13) + dependency-free genrule
  ...
  Defects: stub `WORKSPACE.bazel` present, `--noenable_bzlmod` in `.bazelrc` (D6)
  (resolved by #426: WORKSPACE.bazel deleted, .bazelrc bzlmod-only), no
