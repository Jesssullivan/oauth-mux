...
| --- | --- | --- |
| Historical implementation below the product anchor | Sections 1 and later of `docs/spec/broker-mcp-contract-2026-05-03.md`: broker-daemon diagram, MCP `surface_version: 1`, adapter catalog, and May phase plan | Keep Section 0 as the immutable success metric. Label the later sections historical immediately and migrate any still-binding shipped invariants into the active v0.2 plan, managed-harness v2 contract, or Codex adapter spec. Remove or split the stale implementation prose only after reference migration; Git and immutable evidence retain its history. It may never outrank the v0.2 program. |
...
| Retired npm staging and dead workflows | `dist/npm/`; npm staging in `scripts/release-local.sh`, `scripts/release-smoke.sh`, and `scripts/release-handoff.sh`; `scripts/npm-ci-publish.sh`; `.github/workflows/npm-publish.yml`; retired-lane instructions in `docs/release-runbook.md` and `docs/registry-dry-runs-and-rollback.md` | Characterize the supported v0.1.15 release tree, then remove tarball staging, smoke assumptions, publish script, and dead publish workflow in the first subtraction slice. Add a reintroduction guard and require the future Zig-emitted manifest to keep npm absent. Keep `.github/workflows/npm-deprecate.yml` and `scripts/npm-ci-deprecate.sh` only while registry deprecation maintenance is needed; preserve historical release records. |
