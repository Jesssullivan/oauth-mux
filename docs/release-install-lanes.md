...
| v0.2 POSIX source install contract | `just v02-posix-install-contract-local` | no | Nix supplies the Python/Git/Zig source-test tools. The raw named graph is `local_debug_only`; exact binding is restricted to a Nix-generated closure-bound helper and requires an independent clean-tree rebuild and byte comparison. The noninstalling Zig step materializes a real two-member candidate archive and exercises transactional installation in injected temporary roots. `host_install_enabled` remains false; normal v0.1.15 install/release graphs, PATH winners, stable aliases, and native `codex` remain untouched. |
