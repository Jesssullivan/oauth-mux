...
| Wire cassette coverage | TIN-950 | #176 | PR #299 promoted a scrubbed current-release auth-failure fixture and replay smoke. Linear-Done scope covers the shipped auth-failure fixtures and replay smoke; GitHub #176 stays open for the remaining quota/rate-limit and all-fallbacks-exhausted cassettes. Anthropic-side coverage is re-pointed to TIN-2722; Codex-side coverage is re-pointed to #212/TIN-1824. |
...
| Stay-afloat valet ladder | TIN-2719 / TIN-2720 / TIN-2721 | #445 | M0 advisor in progress; M1 managed switch is gated on TIN-2721 evidence (TIN-2077); M3 request-path mux is explicitly possibly-never — TIN-2040 stays open as a Codex wire-proxy perf lane, parked as golden path. |
| Keepalive service residency | TIN-1830 (Done) / TIN-2723 | — | Residency proven (`#444`, `docs/evidence/keepalive-service-residency-20260704T161515Z/`); TIN-2723 covers doctor version/SHA visibility, stale-warn, PATH-shadow, and the systemd leg; escalates if a switch increment merges. |
